...
let loginUser = async(req, res) => {
        ...
        try {
            ...
            if (!isValid(email)) {
                return res.status(400).send({ status: false, message: "Email is requrid" })

            }

            if (!(/^\w+([\.-]?\w+)@\w+([\.-]?\w+)(\.\w{2,3})+$/).test(email)) {
                return res.status(400).send({ status: false, message: "Please mention a valid Email" })

            }
            if (!isValid(password)) {
                return res.status(400).send({ status: false, message: "Password is requrid" })
